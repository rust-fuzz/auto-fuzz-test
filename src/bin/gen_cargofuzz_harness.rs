...
fn main() -> io::Result<()> {
    ...
    if std::env::args().skip(1).next().is_none() {
        eprintln!("Usage: {} path [path ...]", std::env::args().next().unwrap());
        std::process::exit(2);
    }
    ...
    let crate_info: CrateInfo = {
        let mut rust_info;
        let path_str = std::env::args().skip(1).next().expect("crate path required");
    ...
    let syntax_tree: syn::File = syn::parse_str(&code_str)
        .expect(&format!("Crate '{:?}': not Rust code, does the crate compile?",
    ...
    let callback = |this: Option<&Type>, ident: &Ident, decl: &FnDecl,
                    unsafety: &Option<Unsafe>, asyncness: &Option<Async>,
        ...
        if unsafety.is_none() && asyncness.is_none() {
            let mut fn_inv = BufWriter::new(
                File::create(crate_info.fuzz_dir().unwrap().join(
                        format!("{}.rs", &ident.to_string()))).unwrap());
            println!("{:?}", ident);
            write_fn_invocation(&mut fn_inv, this, ident, decl, crate_info.crate_name()).unwrap();
        }
    };

    FnVisitor{callback: Box::new(callback), context: crate_info}.visit_file(&syntax_tree);
    Ok(())
}

fn write_fn_invocation(
    mut result: &mut Write,
    this: Option<&Type>,
    ident: &Ident,
    decl: &FnDecl,
    crate_name: &str
    ...
    let (crate_prefix, crate_suffix) = {
        ...
        (template_split.next().unwrap(),
         template_split.next().expect("need '{crate_name}' in template"))
    };

    // write the beginning of the template up to the {0} insertion point
    write!(&mut result, "{}{}{}", crate_prefix, crate_name_undescored, crate_suffix)?;
