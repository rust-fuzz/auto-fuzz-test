...
pub struct FnVisitor<T> {
    pub callback: Box<FnMut(Option<&Type>,
                            &Ident, &FnDecl,
                            &Option<Unsafe>, &Option<Async>, &T) -> ()>,
    pub context: T
}
...
impl<'ast, T> Visit<'ast> for FnVisitor<T> {
    ...
    fn visit_item_fn(&mut self, f: &'ast ItemFn) {
        (self.callback)(None, &f.ident, &*f.decl, &f.unsafety, &f.asyncness, &self.context);
    ...
    fn visit_item_impl(&mut self, f: &'ast ItemImpl) {
        ...
        for item in &f.items {
            if let ImplItem::Method(f) = item {
                (self.callback)(Some(self_type), &f.sig.ident, &f.sig.decl, &f.sig.unsafety, &f.sig.asyncness, &self.context);
            }
        }
